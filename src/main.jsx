...
/** ---------- router ---------- */
const ROUTES = [
  "login","register","profile","rating","stats","add",
...
function canAccess(path, userDoc){
  ...
  if (!isAuth) return path === "login" || path === "register";
...
async function render(){
  ...
  mount("mount-register", show("register") ? <ErrorBoundary name="register"><PageRegister/></ErrorBoundary> : null);
...
function Guard(){
  return (
    <div className="glass card">
      ...
      <p className="p">Войдите или зарегистрируйтесь.</p>
      <div className="sep"></div>
      <div style={{display:"flex", gap:10, flexWrap:"wrap"}}>
        <Btn kind="primary" onClick={()=>navigate("login")}>Войти</Btn>
        <Btn onClick={()=>navigate("register")}>Регистрация</Btn>
      </div>
    </div>
  );
}
...
function SidebarNav(){
  ...
  const list = !u ? [
    ...
    {p:"register", t:"Регистрация", i:"plus"},
      ...
      <div className="glass card">
        ...
        <form onSubmit={submit}>
          ...
          <div style={{display:"flex", gap:10, flexWrap:"wrap", marginTop:12}}>
            ...
            <Btn type="button" onClick={()=>navigate("register")}>Регистрация</Btn>
          </div>
        </form>
      </div>
  ...
  return (
    <div className="grid2">
      <div className="glass card">
        ...
        <p className="p">Заполни базовые данные — остальное можно позже.</p>
        <div className="sep"></div>
        <form onSubmit={submit}>
          ...
          <Input type="number" min="0" max="60" value={years} onChange={(e)=>setYears(e.target.value)} />
          <div style={{display:"flex", gap:10, flexWrap:"wrap", marginTop:12}}>
            <Btn kind="primary" type="submit" disabled={st.loading}>Создать</Btn>
            <Btn type="button" onClick={()=>navigate("login")}>Уже есть</Btn>
          </div>
        </form>
      </div>
      ...
      <div className="glass card">
        ...
        <div className="sep"></div>
        <p className="p">После регистрации можно добавить аватар, телефон, город, должность.</p>
      </div>
    </div>
  );
}
